...
#include "PDGameplayAbility_RangedWeapon.h"
#include "Game/Weapons/PDRangedWeaponInstance.h"
#include "Game/Physics/PDCollisionChannels.h"
#include "Game/PDLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Game/Weapons/PDWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "Game/AbilitySystem/PDGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"
#include "Game/Combat/Component/PDPlayerCombatComponent.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PDGameplayAbility_RangedWeapon)
...
namespace PDConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("pd.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("pd.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("pd.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UPDGameplayAbility_RangedWeapon::UPDGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UPDRangedWeaponInstance* UPDGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UPDRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UPDGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogPDAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UPDRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UPDGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UPDGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UPDGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return PD_TraceChannel_Weapon;
}

FHitResult UPDGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UPDGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UPDGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EPDAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UPDWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UPDWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EPDAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EPDAbilityTargetingSource::CameraTowardsFocus) || (Source == EPDAbilityTargetingSource::PawnTowardsFocus) || (Source == EPDAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EPDAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EPDAbilityTargetingSource::WeaponForward) || (Source == EPDAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else if (Source == EPDAbilityTargetingSource::TraceNearEnemyForward)
	{
		if (UPDPlayerCombatComponent * PlayerCombatCompo =  UPDPlayerCombatComponent::FindPlayerCombatComponent(SourcePawn->GetController()))
		{
			SourceLoc = PlayerCombatCompo->TraceNearEnemy();
		}		
	}
	else
	{
	...
	if (bFoundFocus && ((Source == EPDAbilityTargetingSource::PawnTowardsFocus) || (Source == EPDAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UPDGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (PDConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, PDConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UPDGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UPDRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EPDAbilityTargetingSource::TraceNearEnemyForward);
		...
		if (PDConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, PDConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UPDGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UPDRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (PDConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, PDConsoleVariables::DrawBulletHitRadius, FColor::Red, false, PDConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UPDGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UPDGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UPDGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UPDWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPDWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogPDAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UPDGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UPDWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UPDWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FPDGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FPDGameplayAbilityTargetData_SingleTargetHit();
