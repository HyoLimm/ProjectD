...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PDWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

UPDWeaponInstance::UPDWeaponInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (UPDHealthComponent* HealthComponent = UPDHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void UPDWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UPDWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void UPDWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float UPDWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
TSubclassOf<UAnimInstance> UPDWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
	const FPDAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
	return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId UPDWeaponInstance::GetOwningUserId() const
{
...
void UPDWeaponInstance::ApplyDeviceProperties()
{
...
void UPDWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void UPDWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
