...
#include "PDWeaponStateComponent.generated.h"
...
struct FPDScreenSpaceHitLocation
{
...
struct FPDServerSideHitMarkerBatch
{
	FPDServerSideHitMarkerBatch() { }

	FPDServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FPDScreenSpaceHitLocation> Markers;
...
UCLASS()
class UPDWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:
	UPDWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintPure, Category = "PD|WeaponState")
	static UPDWeaponStateComponent* FindWeaponStateComponent(const AActor* Actor) { return (Actor ? Actor->FindComponentByClass<UPDWeaponStateComponent>() : nullptr); }
	...
	void GetLastWeaponDamageScreenLocations(TArray<FPDScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FPDScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FPDServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
