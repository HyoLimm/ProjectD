...
#include "PDWeaponStateComponent.h"
...
#include "Game/Physics/PhysicalMaterialWithTags.h"
//#include "Teams/PDTeamSubsystem.h"
#include "Game/Weapons/PDRangedWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PDWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

UPDWeaponStateComponent::UPDWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void UPDWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

	if (APawn* Pawn = GetPawn<APawn>())
	{
		if (UPDEquipmentManagerComponent* EquipmentManager = Pawn->FindComponentByClass<UPDEquipmentManagerComponent>())
		{
			if (UPDRangedWeaponInstance* CurrentWeapon = Cast<UPDRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(UPDRangedWeaponInstance::StaticClass())))
			{
...
bool UPDWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
	AActor* HitActor = Hit.GetActor();

	//@TODO: Don't treat a hit that dealt no damage (due to invulnerability or similar) as a success
	UWorld* World = GetWorld();
	/*if (UPDTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<UPDTeamSubsystem>(GetWorld()))
	{
...
bool UPDWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void UPDWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FPDServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FPDScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void UPDWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FPDServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FPDScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void UPDWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void UPDWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double UPDWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();
