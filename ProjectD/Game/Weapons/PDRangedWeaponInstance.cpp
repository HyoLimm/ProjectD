...
#include "PDRangedWeaponInstance.h"
#include "NativeGameplayTags.h"
#include "GameFramework/Pawn.h"
#include "GameFramework/CharacterMovementComponent.h"

#include "Game/Physics/PhysicalMaterialWithTags.h"
#include "Game/Weapons/PDWeaponInstance.h"
#include "Game/Character/Component/PDCameraComponent.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(PDRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_PD_Weapon_SteadyAimingCamera, "PD.Weapon.SteadyAimingCamera");

UPDRangedWeaponInstance::UPDRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UPDRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UPDRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UPDRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UPDRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UPDRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UPDRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UPDRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UPDRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UPDRangedWeaponInstance::AddSpread()
{
...
float UPDRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UPDRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UPDRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UPDRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UPDCameraComponent* CameraComponent = UPDCameraComponent::FindCameraComponent(Pawn))
	{
		float TopCameraWeight;
		FGameplayTag TopCameraTag;
		CameraComponent->GetBlendInfo(/*out*/ TopCameraWeight, /*out*/ TopCameraTag);

		AimingAlpha = (TopCameraTag == TAG_PD_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
