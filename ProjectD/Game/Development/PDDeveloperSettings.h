...
#include "PDDeveloperSettings.generated.h"
...
class UPDDeveloperSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UPDDeveloperSettings();
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PD, meta=(AllowedTypes="PDExperienceDefinition"))
	FPrimaryAssetId ExperienceOverride;

	UPROPERTY(BlueprintReadOnly, config, Category=PDBots)
	bool bOverrideBotCount = false;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PDBots, meta=(EditCondition=bOverrideBotCount))
	int32 OverrideNumPlayerBotsToSpawn = 0;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PDBots)
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, config, Category=PD)
	bool bTestFullGameFlowInPIE = false;

	/**
	* Should force feedback effects be played, even if the last input device was not a gamepad?
	* The default behavior in PD is to only play force feedback if the most recent input device was a gamepad.
	*/
	UPROPERTY(config, EditAnywhere, Category = PD, meta = (ConsoleVariable = "PDPC.ShouldAlwaysPlayForceFeedback"))
	...
	UPROPERTY(config, EditAnywhere, Category=PD)
	TArray<FPDCheatToRun> CheatsToRun;
