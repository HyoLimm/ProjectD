...
#include "PDInputModifier.generated.h"

/**
*  Scales input basedon a double property in the SharedUserSettings
*/
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "PD Setting Based Scalar"))
class UPDSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EPDTargetingType : uint8
{
...
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category = Settings, Config)
	EPDTargetingType TargetingType = EPDTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	//UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (AssetBundles = "Client,Server"))
	//TObjectPtr<const UPDAimSensitivityData> SensitivityLevelTable;
