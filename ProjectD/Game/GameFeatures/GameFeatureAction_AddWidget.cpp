...
#define LOCTEXT_NAMESPACE "PDGameFeatures"
...
void UGameFeatureAction_AddWidgets::OnGameFeatureDeactivating(FGameFeatureDeactivatingContext& Context)
{
	...
	FPerContextData* ActiveData = _ContextData.Find(Context);
...
{
	for (const FPDHUDElementEntry& Entry : Widgets)
	{
...
EDataValidationResult UGameFeatureAction_AddWidgets::IsDataValid(FDataValidationContext& Context) const
{
	...
	{
		int32 EntryIndex = 0;
		for (const FPDHUDLayoutRequest& Entry : Layout)
		{
...
void UGameFeatureAction_AddWidgets::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
	...
	FPerContextData& ActiveData = _ContextData.FindOrAdd(ChangeContext);

	if ((GameInstance != nullptr) && (World != nullptr) && World->IsGameWorld())
	{
		if (UGameFrameworkComponentManager* ComponentManager = UGameInstance::GetSubsystem<UGameFrameworkComponentManager>(GameInstance))
		{			
			TSoftClassPtr<AActor> HUDActorClass = APDHUD::StaticClass();
...
{
	APDHUD* HUD = CastChecked<APDHUD>(Actor);
