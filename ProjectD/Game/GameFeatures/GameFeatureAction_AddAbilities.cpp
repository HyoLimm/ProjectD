...
//#include "Player/PDPlayerState.h" //@TODO: For the fname
...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const FPDAbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const FPDAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		//for (const TSoftObjectPtr<const UPDAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		//{
...
void UGameFeatureAction_AddAbilities::HandleActorExtension(AActor* Actor, FName EventName, int32 EntryIndex, FGameFeatureStateChangeContext ChangeContext)
{
	...
	if (AbilitiesList.IsValidIndex(EntryIndex) && ActiveData)
	{
		...
		//else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == APDPlayerState::NAME_PDAbilityReady))
		//{
...
//
//		for (const FPDAbilityGrant& Ability : AbilitiesEntry.GrantedAbilities)
//		{
//			if (!Ability.AbilityType.IsNull())
//			{
...
//		}
//
//		for (const FPDAttributeSetGrant& Attributes : AbilitiesEntry.GrantedAttributes)
//		{
//			if (!Attributes.AttributeSetType.IsNull())
//			{
...
//		}
//
////		UPDAbilitySystemComponent* PDASC = CastChecked<UPDAbilitySystemComponent>(AbilitySystemComponent);
////		for (const TSoftObjectPtr<const UPDAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
////		{
////			if (const UPDAbilitySet* Set = SetPtr.Get())
////			{
//////				Set->GiveToAbilitySystem(PDASC, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
////			}
////		}
//
...
////
//////			UPDAbilitySystemComponent* PDASC = CastChecked<UPDAbilitySystemComponent>(AbilitySystemComponent);
///////			for (FPDAbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
//////			{
//////				SetHandle.TakeFromAbilitySystem(PDASC);
//////			}
////		}
//
