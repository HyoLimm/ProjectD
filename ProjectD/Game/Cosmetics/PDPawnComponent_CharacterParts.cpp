...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PDPawnComponent_CharacterParts)
...
FString FPDAppliedCharacterPartEntry::GetDebugString() const
{
...
void FPDCharacterPartList::PreReplicatedRemove(const TArrayView<int32> RemovedIndices, int32 FinalSize)
{
	bool bDestroyedAnyActors = false;
	for (int32 Index : RemovedIndices)
	{
		FPDAppliedCharacterPartEntry& Entry = Entries[Index];
...
void FPDCharacterPartList::PostReplicatedAdd(const TArrayView<int32> AddedIndices, int32 FinalSize)
{
...
void FPDCharacterPartList::PostReplicatedChange(const TArrayView<int32> ChangedIndices, int32 FinalSize)
{
...
FPDCharacterPartHandle FPDCharacterPartList::AddEntry(FPDCharacterPart NewPart)
{
	FPDCharacterPartHandle Result;
	Result.PartHandle = PartHandleCounter++;

	if (ensure(OwnerComponent && OwnerComponent->GetOwner() && OwnerComponent->GetOwner()->HasAuthority()))
	{
		FPDAppliedCharacterPartEntry& NewEntry = Entries.AddDefaulted_GetRef();
...
void FPDCharacterPartList::RemoveEntry(FPDCharacterPartHandle Handle)
{
	for (auto EntryIt = Entries.CreateIterator(); EntryIt; ++EntryIt)
	{
		FPDAppliedCharacterPartEntry& Entry = *EntryIt;
...
void FPDCharacterPartList::ClearAllEntries(bool bBroadcastChangeDelegate)
{
	bool bDestroyedAnyActors = false;
	for (FPDAppliedCharacterPartEntry& Entry : Entries)
	{
		bDestroyedAnyActors |= DestroyActorForEntry(Entry);
	}
	Entries.Reset();
...
FGameplayTagContainer FPDCharacterPartList::CollectCombinedTags() const
{
	FGameplayTagContainer Result;

	for (const FPDAppliedCharacterPartEntry& Entry : Entries)
	{
...
bool FPDCharacterPartList::SpawnActorForEntry(FPDAppliedCharacterPartEntry& Entry)
{
	bool bCreatedAnyActors = false;
...
bool FPDCharacterPartList::DestroyActorForEntry(FPDAppliedCharacterPartEntry& Entry)
{
	bool bDestroyedAnyActors = false;
...
UPDPawnComponent_CharacterParts::UPDPawnComponent_CharacterParts(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SetIsReplicatedByDefault(true);
}

void UPDPawnComponent_CharacterParts::GetLifetimeReplicatedProps(TArray< FLifetimeProperty >& OutLifetimeProps) const
{
...
FPDCharacterPartHandle UPDPawnComponent_CharacterParts::AddCharacterPart(const FPDCharacterPart& NewPart)
{
	return CharacterPartList.AddEntry(NewPart);
}

void UPDPawnComponent_CharacterParts::RemoveCharacterPart(FPDCharacterPartHandle Handle)
{
	CharacterPartList.RemoveEntry(Handle);
}

void UPDPawnComponent_CharacterParts::RemoveAllCharacterParts()
{
	CharacterPartList.ClearAllEntries(/*bBroadcastChangeDelegate=*/ true);
}

void UPDPawnComponent_CharacterParts::BeginPlay()
{
	Super::BeginPlay();
}

void UPDPawnComponent_CharacterParts::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UPDPawnComponent_CharacterParts::OnRegister()
{
	Super::OnRegister();
...
TArray<AActor*> UPDPawnComponent_CharacterParts::GetCharacterPartActors() const
{
	TArray<AActor*> Result;
	Result.Reserve(CharacterPartList.Entries.Num());

	for (const FPDAppliedCharacterPartEntry& Entry : CharacterPartList.Entries)
	{
...
USkeletalMeshComponent* UPDPawnComponent_CharacterParts::GetParentMeshComponent() const
{
...
USceneComponent* UPDPawnComponent_CharacterParts::GetSceneComponentToAttachTo() const
{
...
FGameplayTagContainer UPDPawnComponent_CharacterParts::GetCombinedTags(FGameplayTag RequiredPrefix) const
{
...
void UPDPawnComponent_CharacterParts::BroadcastChanged()
{
