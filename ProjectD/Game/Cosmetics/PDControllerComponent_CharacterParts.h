...
#include "PDControllerComponent_CharacterParts.generated.h"
...
USTRUCT()
struct FPDControllerCharacterPartEntry
{
	GENERATED_BODY()

	FPDControllerCharacterPartEntry()
	{}
...
class UPDControllerComponent_CharacterParts : public UControllerComponent
{
	GENERATED_BODY()

public:
	UPDControllerComponent_CharacterParts(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void AddCharacterPart(const FPDCharacterPart& NewPart);
	...
	UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category=Cosmetics)
	void RemoveCharacterPart(const FPDCharacterPart& PartToRemove);
...
protected:
	...
	TArray<FPDControllerCharacterPartEntry> CharacterParts;

private:
	UPDPawnComponent_CharacterParts* GetPawnCustomizer() const;

	UFUNCTION()
	void OnPossessedPawnChanged(APawn* OldPawn, APawn* NewPawn);

	void AddCharacterPartInternal(const FPDCharacterPart& NewPart, ECharacterPartSource Source);

	void AddCheatPart(const FPDCharacterPart& NewPart, bool bSuppressNaturalParts);
	void ClearCheatParts();

	void SetSuppressionOnNaturalParts(bool bSuppressed);

	friend class UPDCosmeticCheats;
};
