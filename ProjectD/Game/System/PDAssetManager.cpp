...
const UPDGameData& UPDAssetManager::GetGameData()
{
	return GetOrLoadTypedGameData<UPDGameData>(PDGameDataPath);
}
...
UPrimaryDataAsset* UPDAssetManager::LoadGameDataOfClass(TSubclassOf<UPrimaryDataAsset> DataClass, const TSoftObjectPtr<UPrimaryDataAsset>& DataClassPath, FPrimaryAssetType PrimaryAssetType)
{
...
#endif
		UE_LOG(LogPD, Log, TEXT("Loading GameData: %s ..."), *DataClassPath.ToString());
	...
	else
	{
		...
		UE_LOG(LogPD, Fatal, TEXT("Failed to load GameData asset at %s. Type %s. This is not recoverable and likely means you do not have the correct data to run %s."), *DataClassPath.ToString(), *PrimaryAssetType.ToString(), FApp::GetProjectName());
	}
